...
| :--- | :--- | :--- |
| `max_health` & `health_mult` | Maximum HP capacity. | Maximum HP capacity. |
| `speed` & `speed_mult` | Walking / Movement Speed. | Conveyor belt / transport stream speed. |
| `process_speed` & `process_speed_mult` | Rate of weapon fire or melee strikes. | Machine crafting/processing tick speed. |
...
| `defense` & `defense_mult` | Armor rating (flat damage reduction). | Armor rating (flat damage reduction). |
| `firewall` | Multiplier scaling resistance to elemental debuffs. | Resistance to environmental/elemental damage. |
| `networking` | Magic damage scaling for spells, reactions, and ticks. | Magic damage scaling for spells, reactions, and ticks & increase elemental-based production. |
| `security` | Gives a separate flat health bar that can regenerate. No defenses are applied to it. | Gives a separate flat health bar that can regenerate. No defenses are applied to it. |
| `space` & `space_mult` | Physics mass. High space(weight) resists ragdolling. | Environmental effects check (gimicky). |
| `luck_stat` | Critical hit chance and dodge percentage. | Percentage chance to produce double outputs. |
| `compute` | Stamina / Energy consumption rate. | Power grid drain divisor (2.0 = uses 50% power). |
| `scale`(entity_size to not conflict) | Size of entity/attack | Increase size of output |
| `ping` | Mult against CC duration | Mult against CC duration |
| `malware` | Vulnerability/Difficulty (+10% extra damage taken). | Vulnerability (+10% extra damage taken). |
