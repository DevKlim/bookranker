...
func _physics_process(delta: float) -> void:
	...
	if Engine.has_singleton("LaneManager"):
		...
		if is_instance_valid(wire):
			...
			if get_meta("sea_borne", false) and (is_on_water or is_on_ink):
				# Force the seaborne projectile (like Swans) to follow the stream's placed direction
				if "output_direction" in wire:
					var dir_idx = int(wire.output_direction)
	...
	if get_meta("sea_borne", false) and not is_on_water and not is_on_ink:
		...
		var grace = get_meta("grace_period", 0.0)
		if grace > 0:
	...
	if get_meta("lifetime"):
		var lf = get_meta("lifetime")
		lf -= delta
	...
	# Swan on Tarstream (Ticking Damage processing)
	if get_meta("tick_damage", false):
		_tick_timer -= delta
		if _tick_timer <= 0:
			_tick_timer = 1.0
